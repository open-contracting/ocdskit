...
def test_command(monkeypatch):
    assert_command(monkeypatch, main, ['package-records', '--uri', 'http://example.com', '--published-date',
...
def test_command_root_path_array(monkeypatch):
    assert_command(monkeypatch, main, ['package-records', '--root-path', 'records'],
...
def test_command_root_path_item(monkeypatch):
    assert_command(monkeypatch, main, ['package-records', '--root-path', 'records.item'],
                   ['realdata/record-package-1.json', 'realdata/record-package-2.json'],
                   ['realdata/record-package_record-package.json'])
