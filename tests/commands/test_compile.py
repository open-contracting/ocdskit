...
def test_command_extensions_with_releases(capsys, monkeypatch):
    assert_streaming(
        ...
        monkeypatch,
        main,
        ["compile", "--root-path", "releases.item"],
        ["release-package_additional-contact-points.json"],
        ["compile_no-extensions.json"],
    )
...
def test_command_package_fake(capsys, monkeypatch):
    actual = run_streaming(
        capsys, monkeypatch, main, ["compile", "--package", "--fake"], ["release-package_minimal.json"]
    )
...
def test_command_package_packages(capsys, monkeypatch):
    actual = run_streaming(capsys, monkeypatch, main, ["compile", "--package"], ["release_minimal.json"])
...
def test_command_warning(capsys, monkeypatch, caplog):
    actual = run_streaming(capsys, monkeypatch, main, ["compile"], ["release-package_warning.json"])
...
def test_command_package_linked_releases_with_packages(capsys, monkeypatch):
    assert_streaming(
        ...
        monkeypatch,
        main,
        ["compile", "--package", "--linked-releases"],
        ["realdata/release-package-1.json", "realdata/release-package-2.json"],
...
def test_command_package_linked_releases_with_releases(capsys, monkeypatch):
    assert_streaming(
        ...
        monkeypatch,
        main,
        ["compile", "--package", "--linked-releases", "--root-path", "releases.item"],
        ["realdata/release-package-1.json", "realdata/release-package-2.json"],
...
def test_command_version_mismatch(capsys, monkeypatch, caplog):
    with caplog.at_level(logging.ERROR):
        ...
        assert caplog.records[0].message == (
            "item 1: version error: this item uses version 1.0, but earlier items used version 1.1\n"
...
def test_command_without_sqlite(capsys, monkeypatch, caplog):
    ...
    assert caplog.records[0].message == (
        "sqlite3 is unavailable, so the command will run in memory. "
        "If input files are too large, the command might exceed available memory."
    )
