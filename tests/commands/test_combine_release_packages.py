...
def test_command(monkeypatch):
    assert_command(monkeypatch, main, ['combine-release-packages'],
...
def test_command_uri_published_date(monkeypatch):
    actual = run_command(monkeypatch, main, ['combine-release-packages', '--uri', 'http://example.com/x.json',
...
def test_command_publisher(monkeypatch):
    actual = run_command(monkeypatch, main, ['combine-release-packages', '--publisher-name', 'Acme Inc.'],
                         ['release-package_minimal.json'])
