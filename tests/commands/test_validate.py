...
from tests import assert_command_raises, read, run_command


@pytest.mark.vcr()
def test_command(monkeypatch):
    actual = run_command(monkeypatch, main, ['validate'],
...
def test_command_invalid_json(monkeypatch, caplog):
    with caplog.at_level(logging.INFO):
        ...
        actual, excinfo = assert_command_raises(monkeypatch, main, ['validate'], stdin)
...
def test_command_valid_release_package_url(monkeypatch):
    ...
    actual = run_command(monkeypatch, main, ['validate', '--schema', url],
                         ['realdata/release-package-1.json'])
...
def test_command_valid_release_package_file_verbose(monkeypatch):
    ...
    actual = run_command(monkeypatch, main, ['validate', '--schema', url, '--verbose'],
                         ['realdata/release-package-1.json'])
