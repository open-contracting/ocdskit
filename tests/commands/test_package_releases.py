...
def test_command(capsys, monkeypatch):
    assert_streaming(capsys, monkeypatch, main, ['package-releases', '--uri', 'http://example.com', '--published-date',
...
def test_command_size(capsys, monkeypatch):
    assert_streaming(capsys, monkeypatch, main, ['package-releases', '--size', '2'],
...
def test_command_root_path_array(capsys, monkeypatch):
    assert_streaming(capsys, monkeypatch, main, ['package-releases', '--root-path', 'records.item.releases'],
                     ['realdata/record-package-1.json', 'realdata/record-package-2.json'],
                     ['realdata/release-package_record-package.json'])


def test_command_root_path_item(capsys, monkeypatch):
    assert_streaming(capsys, monkeypatch, main, ['package-releases', '--root-path', 'records.item.releases.item'],
                     ['realdata/record-package-1.json', 'realdata/record-package-2.json'],
                     ['realdata/release-package_record-package.json'])
