...
Users on Windows should run ``set PYTHONIOENCODING=utf-8`` in each terminal session before running any ``ocdskit`` commands. To set this environment variable for all future sessions, run ``setx PYTHONIOENCODING utf-8``.
